...
ctrl.like = async (req, res) => {
  ...
  if (image) {
    const user = await Image.findOne({
      $and: [{ filename: { $regex: req.params.image_id } },
      { userslikes: req.user.email }]
    });
    if (!user) {
      //likes por usuario
      await Image.update(
        { filename: { $regex: req.params.image_id } }, { $addToSet: { userslikes: req.user.email } })
      image.likes = image.likes + 1;
    } else {
      await Image.update(
        { filename: { $regex: req.params.image_id } }, { $pull: { userslikes: req.user.email } })
      image.likes = image.likes - 1;
    }
    await image.save();
